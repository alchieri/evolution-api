    ...
    try {
      ...
      if (instance.state === 'connecting' || instance.state === 'open') {
        try {
          ...
          // logout can throw when the underlying socket is already dead but the
          // in-memory instance still exists. Continue to remove.instance so the
          // stale entry is purged and cleanup runs.
          this.logger.warn({
